...
import br.alandoni.pdfcompare.android.env.ConfigFileEnvironment;
import br.alandoni.pdfcompare.android.env.DefaultEnvironment;
import br.alandoni.pdfcompare.android.env.Environment;
import br.alandoni.pdfcompare.android.env.SimpleEnvironment;

import static br.alandoni.pdfcompare.android.Utilities.blockingExecutor;
...
public class PdfComparator<T extends CompareResultImpl> {
        ...
        try {
            ...
            try (final InputStream expectedStream = expectedStreamSupplier.get()) {
                try (final InputStream actualStream = actualStreamSupplier.get()) {
                    try (PDDocument expectedDocument = PDDocument.load(
                            expectedStream, expectedPassword, Utilities.getMemorySettings(environment.getDocumentCacheSize()))) {
                        try (PDDocument actualDocument = PDDocument.load(
                                actualStream, actualPassword, Utilities.getMemorySettings(environment.getDocumentCacheSize()))) {
                            compare(expectedDocument, actualDocument);
                        }
                    }
                } catch (NoSuchFileException ex) {
                    addSingleDocumentToResult(expectedStream, environment.getActualColor());
                    compareResult.expectedOnly();
                }
            } catch (NoSuchFileException ex) {
                try (final InputStream actualStream = actualStreamSupplier.get()) {
                    addSingleDocumentToResult(actualStream, environment.getExpectedColor());
    ...
    private void compare(final PDDocument expectedDocument, final PDDocument actualDocument) throws IOException {
        ...
        if (expectedDocument.getNumberOfPages() > minPageCount) {
            addExtraPages(expectedDocument, expectedPdfRenderer, minPageCount, environment.getActualColor(), true);
        } else if (actualDocument.getNumberOfPages() > minPageCount) {
            addExtraPages(actualDocument, actualPdfRenderer, minPageCount, environment.getExpectedColor(), false);
        }
    ...
    private void addExtraPages(final PDDocument document, final PDFRenderer pdfRenderer, final int minPageCount,
        ...
        for (int pageIndex = minPageCount; pageIndex < document.getNumberOfPages(); pageIndex++) {
            ...
            for (int i = 0; i < image.bufferedImage.getWidth() * MARKER_WIDTH; i++) {
                x++;
                if (x > image.bufferedImage.getWidth()) {
                    y++;
                    x = 0;
                }
                image.bufferedImage.setPixel(x, y, color);
            }
            for (int i = 0; i < image.bufferedImage.getHeight(); i++) {
                for (int j = 0; j < MARKER_WIDTH; j++) {
                    image.bufferedImage.setPixel(j, i, color);
                }
            }
            if (expected) {
    ...
    private static ImageWithDimension blank(final ImageWithDimension image) {
        return new ImageWithDimension(
                Bitmap.createBitmap(image.bufferedImage.getWidth(), image.bufferedImage.getHeight(), image.bufferedImage.getConfig()),
    ...
    public static ImageWithDimension renderPageAsImage(final PDDocument document, final PDFRenderer expectedPdfRenderer, final int pageIndex,
        ...
        final Bitmap bufferedImage = expectedPdfRenderer.renderImageWithDPI(pageIndex, environment.getDPI());
